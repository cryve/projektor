    ...
    if (Meteor.isServer) {
      ...
      magic.detectFile(fileRef.path, Meteor.bindEnvironment((err, mimeType) => {
        if (err || !~mimeType.indexOf('image')) {
          ...
          console.log('onAfterUpload, not an image: ', fileRef.path);
          console.log('deleted', fileRef.path);
...
if (Meteor.isServer) {
  ...
  Meteor.publish("files.images.single", function singleImagePublication(imageId) {
  ...
  Meteor.publishComposite("files.images.avatar", function avatarPublication(userId) {
    return {
        find() {
  ...
  Meteor.publish("files.images.gallery", function singleImagePublication(media) {
    const imageIds = [];
