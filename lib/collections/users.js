...
if (Meteor.isServer) {
  ...
  Meteor.publish('user.profile.all', function userProfileAllPublication() {
    if (!this.userId) {
      return this.ready();
    }
    return Meteor.users.find({}, {
  ...
  Meteor.publish('user.profile.course', function userProfilecoursePublication(courseId) {
    if (!this.userId) {
      return this.ready();
    }
    const course = Courses.findOne(courseId);
    const courseOwnersAndMembers = lodash.concat(course.owner, course.member);
    console.log(courseOwnersAndMembers);
    return Meteor.users.find({ _id: { $in: courseOwnersAndMembers } }, {
      fields: {
        _id: 1,
        'profile.email': 1,
        'profile.avatar': 1,
  ...
  Meteor.publish('users.list.single', function usersListSinglePublication(userId) {
