...
Meteor.methods({
  'excel'({ courseId}) {
    ...
    if (!lodash.contains(course.owner, this.userId)) {
...
export const insertEmptyCourseDraft = new ValidatedMethod({
  ...
  run({courseId}) {
    ...
    const supervisorRole = Meteor.user().profile.title;
...
export const addSupervisorToCourse = new ValidatedMethod({
  ...
  run({ docId, userId}) {
    ...
    if (courseProjects){
      courseProjects.forEach(function(project) {
        console.log(project)
        Projects.update(project._id, { $push: { supervisors: {userId:userId, role: user.profile.title }}});
      })
    }
...
export const deleteEditableArrayItem = new ValidatedMethod({
  ...
  run({collectionName, docId, arrayField, item}) {
    ...
    if(lodash.includes(["teamCommunication", "jobs", "contacts"], arrayField)) {
      ...
      if(!lodash.includes(doc.permissions.manageMembers, this.userId)) {
        throw new Meteor.Error(collectionName+".editable.deleteArrayItem.unauthorized",
      ...
      if(arrayField == "supervisors") {
        if(doc.courseId && !lodash.includes(doc.permissions.manageCourses, this.userId)) {
          throw new Meteor.Error(collectionName+".editable.deleteArrayItem.unauthorized",
...
export const addCourseToProject = new ValidatedMethod({
  ...
  run({docId, courseId}) {
    ...
    lodash.forEach(course.owner, function(ownerId){
      var user = Meteor.users.findOne(ownerId)
      Projects.update(docId, { $addToSet: {supervisors:{userId: user._id ,role: user.profile.title }, editableBy: user._id}});
...
export const addCourseToDraft = new ValidatedMethod({
  ...
  run({docId, courseId}) {
    ...
    lodash.forEach(course.owner, function(ownerId){
      ...
      console.log(docId);
      var user = Meteor.users.findOne(ownerId)
      Drafts.update(docId, { $addToSet: {supervisors:{userId: user._id ,role: user.profile.title }, editableBy: user._id}});
...
export const updateMemberInDraft = new ValidatedMethod({
  ...
  run({modifier, _id}) {
    ...
    if(draft.team[memberIndex].userId === this.userId) {
      if(modifier["$set"][userIdKey] != this.userId) {
