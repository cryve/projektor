      ...
      courseProjects.forEach(function(project) {
        lodash.forEach(project.team, function(teamMember) {
          ...
          if(user.profile.role == "Student"){
            var projectObject = {Projektname: project.title, "Matr.Nr": user.profile.matricNo, Vorname:user.profile.firstname, Nachname:user.profile.lastname, Studiengang: user.profile.studyCourse.id};
...
export const addCourseToProject = new ValidatedMethod({
  ...
  run({docId, courseId}) {
    ...
    Projects.update(docId, { $addToSet: {supervisors:{userId: supervisorsId ,role: supervisorRole }, editableBy: supervisorsId}});
  }
});
...
export const addCourseToDraft = new ValidatedMethod({
  ...
  run({docId, courseId}) {
    ...
    Drafts.update(docId, { $addToSet: {supervisors:{userId: supervisorsId ,role: supervisorRole }, editableBy: supervisorsId}});
  }
});
