...
Meteor.methods({
  'excel'({ test1, test2, test3 }) {
    new SimpleSchema({
      test1: { type: String },
      test2: { type: String },
      test3: { type: String }
    }).validate({ test1, test2, test3});
    ...
    if (!Meteor.userId()) {
      ...
      var data = [ { name :test1, lastName : test2, isSpider : true } ,
                   { name : test3,  lastName : test1, powers : ["kinetic cards"] }];
      ...
      if(Meteor.isServer){
        ...
        if(XlsFiles.findOne({userId:userId})){
          XlsFiles.remove({userId:userId});
        }
...
export const addCourseToDraft = new ValidatedMethod({
  ...
  run({docId, courseId}) {
    ...
    const supervisors = user.username;
    ...
    Drafts.update(docId, { $push: {supervisors: supervisors }});
  }
});
