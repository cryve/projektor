  ...
  run() {
    ...
    if(Meteor.user().profile.role == "Mitarbeiter"){
      draftId = Drafts.insert({
        supervisors: [{userId: Meteor.userId(), role: Meteor.user().profile.role}],
      });
    } else {
      draftId = Drafts.insert({});
    }
...
export const addCourseToProject = new ValidatedMethod({
  ...
  run({docId, courseId}) {
    ...
    Projects.update(docId, { $addToSet: {supervisors: { userId: supervisorsId, role: supervisorRole }}});
  }
});
...
export const addCourseToDraft = new ValidatedMethod({
  ...
  run({docId, courseId}) {
    ...
    Drafts.update(docId, { $addToSet: { supervisors: { userId: supervisorsId, role: supervisorRole }}});
  }
});
